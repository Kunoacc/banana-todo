...
export async function get<T>(
  ...
  const url = new URL(
    typeof path === 'string' ? path : path(EXTERNAL_RESOURCE_BASE_URL),
    shouldIncludeBaseUrl ? EXTERNAL_RESOURCE_BASE_URL : undefined
  )
...
/**
 * This is the `delete` derivative of the `http` method.
 * It is used to make DELETE requests to internal or external endpoints across the project
 * @param path - the path which DELETE requests are routed to.
 * @param shouldIncludeBaseUrl - To determine if the path is an internal or external path and would require prepending the BASE_PATH
 * @param filters - A set of filters to be applied to the request
 * @param headers - A set of headers to be applied to the request
 * @returns  A promisified instance of `HttpResponse`
